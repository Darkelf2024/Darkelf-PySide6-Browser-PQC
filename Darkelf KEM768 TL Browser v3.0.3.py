...
os.environ["QTWEBENGINE_CHROMIUM_FLAGS"] = "qt.qpa.*=false;qt.webengine.*=false;qt.webenginecontext=false".join([
    "--disable-features=UserAgentClientHint,GreaseUACH,AcceptCHFrame,ClientHintsDPR,ClientHintsPixelRounding,"
    "CookiesWithoutSameSiteMustBeSecure,AutofillServerCommunication,InterestCohortAPI,PrivacySandboxAdsAPIs,"
    "AudioServiceSandbox,WebRTCMediaDevices,NavigatorOnLine,UserAgentClientHint,WebAuthn,HTMLImports,"
    "AudioContext,HardwareConcurrency,IndexedDB,ClientHints,UserAgentReduction,NetworkService,PrefetchPrivacyChanges",
...
When asked to summarize, provide concise, high-signal analysis with clear recommendations."""
    ...
    def export_csv(self, filename="osint_results.csv"):
        if not self.last_osint_results:
            self.results_box.setPlainText("❌ No results to export.")
            return
        ...
        self.results_box.setPlainText(f"📤 OSINT results saved to {filename}")
...
class DarkelfUtils:
    ...
    def _format_osint_panel(self, results: dict, max_urls: int = 20):
        from rich.table import Table
        from rich.panel import Panel
        from rich.text import Text
        ...
        urls = results.get("urls", [])[:max_urls]
        uniq = results.get("unique_domains", [])
        ai_sum = (results.get("ai_summary") or "").strip()
        ai_ana = (results.get("ai_analysis") or "").strip()

        header = Text("OSINT Summary", style="bold white")
        counts = Text(
            f"  • URLs: {len(results.get('urls', []))}  "
            f"• Unique domains: {len(uniq)}",
            style="dim"
        )
        header.append(counts)

        t = Table(show_edge=False, show_header=True, header_style="bold cyan")
        t.add_column("#", justify="right", width=3)
        t.add_column("Domain", style="magenta")
        t.add_column("URL", overflow="fold")

        for i, u in enumerate(urls, 1):
            dom = urlsplit(u).netloc or "-"
            t.add_row(str(i), dom, u)

        dom_table = Table(show_edge=False, show_header=False)
        if uniq:
            for d in uniq:
                dom_table.add_row(f"• {d}")
        else:
            dom_table.add_row("— none —")
    
        outer = Table.grid(padding=(0,1))
        outer.add_row(header)
        outer.add_row("")  # spacer
        outer.add_row(Text("Unique Domains", style="bold green"))
        outer.add_row(dom_table)
        outer.add_row("")  # spacer
        outer.add_row(Text("Top URLs", style="bold green"))
        outer.add_row(t)

        if ai_sum:
            outer.add_row("")  # spacer
            outer.add_row(Text("Darkelf AI Summary", style="bold yellow"))
            outer.add_row(Markdown(ai_sum))
        if ai_ana:
            outer.add_row("")
            outer.add_row(Text("Darkelf AI Analysis", style="bold yellow"))
            outer.add_row(Markdown(ai_ana))

        return Panel(outer, border_style="cyan", title="Darkelf OSINT", subtitle="CLI view")

    def print_osint_to_console(self, results: dict, max_urls: int = 20):
        try:
            panel = self._format_osint_panel(results, max_urls=max_urls)
            self.console.print(panel)
        except Exception as e:
            self.console.print(f"[red]Pretty render failed:[/red] {e}")
    ...
    # NEW: Helper for GUI formatting of AI output
    @staticmethod
    def format_ai_output(text):
        ...
        if isinstance(text, (list, dict)):
            ...
            try:
                ...
                text = json.dumps(text_obj, indent=2)
            except Exception:
                pass
        return "<pre style='font-family:monospace; white-space:pre-wrap;'>" + html.escape(text) + "</pre>"
...
class OsintWorker(QThread):
    ...
    def run(self):
        try:
            utils = DarkelfUtils()
            result = utils.osintscan(self.query, use_tor=True, max_results=10)
            self.result_ready.emit(result)
        except Exception as e:
...
class Darkelf(QMainWindow):
    def __init__(self):
        ...
        self.signals = SignalWrapper(self)
...
"""
    ...
    def launch_osint_scanner(self):
        ...
        self.summarize_button.setProperty("variant", "primary")
        ...
        self.summarize_button.clicked.connect(self._summarize_with_darkelf_ai)  # <-- Fix: will show real summary now
    ...
    def run_osint_query(self):
        ...
        # Use QThread for OSINT scan
        self.osint_worker = OsintWorker(query)
        self.osint_worker.result_ready.connect(self.display_osint_result, Qt.QueuedConnection)
    ...
    def display_osint_result(self, result):
        ...
        if urls:
            ...
            if summary_lines:
                ...
                self.output_box.append("[No results found.]")

        # --- Show full AI summary/analysis below links ---
        ai_summary = result.get("ai_summary", "")
        ai_analysis = result.get("ai_analysis", "")
        if ai_summary:
            self.output_box.append(f"<h4>[Darkelf AI Summary]</h4>{DarkelfUtils.format_ai_output(ai_summary)}")
        if ai_analysis:
            self.output_box.append(f"<h4>[Darkelf AI Analysis]</h4>{DarkelfUtils.format_ai_output(ai_analysis)}")

    def _summarize_with_darkelf_ai(self):
        # When button is pressed, show full AI summary/analysis (not just the count)
        if not self.last_result:
            QMessageBox.information(self.osint_window, "No Results", "Run a scan first.")
            return
        ai_summary = self.last_result.get("ai_summary", "")
        ai_analysis = self.last_result.get("ai_analysis", "")
        output = ""
        if ai_summary:
            output += f"<h4>[Darkelf AI Summary]</h4>{DarkelfUtils.format_ai_output(ai_summary)}"
        if ai_analysis:
            output += f"<h4>[Darkelf AI Analysis]</h4>{DarkelfUtils.format_ai_output(ai_analysis)}"
        if not output:
            output = "[No AI summary available.]"
        self.output_box.append(output)

    @staticmethod
    def _render_osint_html(result):
        urls = result.get("urls", [])
        html_lines = [
        ...
        for u in urls:
            html_lines.append(f'<li><a href="{u}" style="color:#00ff99;">{Darkelf._html_escape(u)}</a></li>')
    ...
    def export_results(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self.osint_window, "Export Results", "", "Text Files (*.txt)"
        )
        if path:
            # Save summary and results as plain text
            lines = self.last_result.get("summary", [])
            with open(path, "w", encoding="utf-8") as f:
                for line in lines:
                    f.write(str(line) + "\n")
            QMessageBox.information(self.osint_window, "Exported", f"Results exported to {path}")
